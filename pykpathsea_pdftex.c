...
static kpse_file_format_type format_abbr(const_string str) {
...
static unsigned find_dpi(string s) {
...
static boolean try_suffix(boolean is_filename, string name, unsigned name_len,
                          const_string ftry) {
  unsigned try_len;
...
static kpse_file_format_type find_format(kpathsea kpse, string name,
                                         boolean is_filename) {
  kpse_file_format_type ret = kpse_last_format;
...
static string lookup(kpathsea kpse, string name) {
  ...
  case kpse_any_glyph_format: {
    ...
    if (!local_dpi)
      local_dpi = dpi;
    ret = kpathsea_find_glyph(kpse, temp, local_dpi, fmt, &glyph_ret);
    if (temp != name)
      free(temp);
  } break;
